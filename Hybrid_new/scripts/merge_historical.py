"""Parse historical tweets and merge with existing database."""
...
HISTORICAL_PATH = Path(__file__).parent.parent / ".mimocode" / "Sources" / "2-hackinga0_ALL_tweets_historical.json.TXT.md"
...
for tweet in historical:
    ...
    # Determine source based on author_id and mentions
    author_id = str(tweet.get("author_id", ""))
    mentions = tweet.get("entities", {}).get("mentions", [])
    mention_usernames = [m.get("username", "") for m in mentions]

    # Target bot = author_id 2051911746969812998 (HackingA0)
