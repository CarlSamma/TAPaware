...
class KnowledgeExpansion:
    ...
    Supports CRUD, countermeasures, import/export (JSON/YAML),
    versioning/history, semantic search, and probe integration.
    """
    ...
    async def add_attack_type(self, attack_type: AttackType) -> AttackType:
        """Add a new attack type with embedding + version history."""
        # Check uniqueness
        ...
        if self.km.vector_store:
            attack_type.embedding = await self.km.vector_store.embedder.encode(embed_text)

        # Insert attack_types row
        ...
        # Also store in KnowledgeMemory for vector recall
        unit = MemoryUnit(
            id=attack_type.id,
            type="knowledge",
            content=embed_text,
            metadata={"attack_type_id": attack_type.id, "category": attack_type.category},
            embedding=attack_type.embedding,
        )
    ...
    async def update_attack_type(
        ...
        """Update attack type with version increment."""
        ...
        if any(k in updates for k in ("name", "description", "category")):
            embed_text = f"{current.name} {current.category} {current.description}"
            if self.km.vector_store:
                current.embedding = await self.km.vector_store.embedder.encode(embed_text)
                await self.km.vector_store.update(current.id, current.embedding)
    ...
    async def list_attack_types(
        ...
        """List attack types with optional filtering."""
        if category:
        ...
        # Tag filtering (post-filter since tags are JSON)
        if tags:
    ...
    async def search_attack_types(
        ...
        """Semantic search over attack types via vector embeddings."""
        results: List[Tuple[AttackType, float]] = []

        if self.km.vector_store:
            for mem_id, score in await self.km.vector_store.search(
                query, top_k=limit, threshold=threshold
            ):
                at = await self.get_attack_type(mem_id)
                if at:
                    results.append((at, score))
    ...
    async def delete_attack_type(self, attack_type_id: str) -> bool:
        """Soft-delete (archive). Creates history entry."""
        ...
        if self.km.vector_store:
            await self.km.vector_store.delete(attack_type_id)
        await self.db.commit()
        return True
    ...
    async def add_countermeasure(
        ...
        """Add a defense/countermeasure to an attack type."""
        at = await self.get_attack_type(attack_type_id)
        if not at:
            raise ValueError(f"Attack type {attack_type_id} not found")
    ...
    async def import_from_json(self, file_path: str) -> int:
        """Import attack types from JSON file. Upsert by name. Returns count."""
        path = Path(file_path)
        if not path.exists():
    ...
    async def import_from_yaml(self, file_path: str) -> int:
        """Import from YAML file (requires pyyaml)."""
        try:
            import yaml
        except ImportError:
    ...
    async def export_to_json(
        ...
        """Export all attack types to JSON."""
    ...
    async def rollback(self, attack_type_id: str, to_version: int) -> AttackType:
        ...
        await self._add_history(result, "rolled_back", extra=f"from v{to_version}")
        await self.db.commit()
        return result
    ...
    async def seed_initial_types(self) -> int:
        """Load V-Genome types from seed file if DB is empty."""
