...
class WorkflowMemory:
    ...
    async def store(self, unit: MemoryUnit) -> MemoryUnit:
        """Store with semantic dedup — skip if too similar to existing."""
        if await self.semantic_dedup(unit.content):
            return unit  # already exists, skip
    ...
    async def recall(
        ...
        """Semantic search for workflow patterns."""
        ...
        if len(results) < limit:
            ...
            rows = await self.db.fetchall(
                """SELECT * FROM memory_units
                   WHERE type = 'workflow' AND content LIKE ?
    ...
    async def delete(self, memory_id: str) -> bool:
        cursor = await self.db.execute(
            "DELETE FROM memory_units WHERE id = ? AND type = 'workflow'",
            (memory_id,),
        )
        if self.vector_store:
    ...
    async def semantic_dedup(self, content: str, threshold: float = 0.85) -> bool:
        ...
        if not self.vector_store:
            return False
        similar = await self.vector_store.search(content, top_k=1, threshold=threshold)
        return len(similar) > 0

    async def count(self) -> int:
        row = await self.db.fetchone(
            "SELECT COUNT(*) as cnt FROM memory_units WHERE type = 'workflow'"
        )
    ...
    async def apply_decay(self) -> int:
        ...
        cursor = await self.db.execute(
            """UPDATE memory_units
               ...
               WHERE type = 'workflow' AND confidence > 0""",
            (now,),
        )
        await self.db.commit()
